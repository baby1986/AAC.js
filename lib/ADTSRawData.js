...
function raw_data_block(bitView, adts, rawData, maxElements) {
    // version 2005
    while (1) {
        var id_syn_ele = bitView["u"](3);
        var id = id_syn_ele;
        if (id !== ID_END) { // ID_END = 0x7
...
function fill_element(bitView, adts, param) {
    ...
    function extension_payload(bitView, cnt) {
        ...
        case EXT_DYNAMIC_RANGE:
            var n = dynamic_range_info(bitView); // TODO: impl
            return n;
        case EXT_SBR_DATA:
        ...
        case EXT_FILL_DATA:
            ...
            var fill_byte = [];
            for (i = 0; i < cnt - 1; i++) {
                fill_byte[i] = bitView["u"](8); // must be '10100101'
            }
            return cnt;
        default:
...
function ics_info(bitView, adts, param) {
    ...
    if (window_sequence === EIGHT_SHORT_SEQUENCE) {
        ...
        if (predictor_data_present) {
            ...
            for (var sfb = 0; sfb < min(max_sfb, PRED_SFB_MAX); sfb++) {
