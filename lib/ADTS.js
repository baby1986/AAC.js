...
var ADTS = {
    ...
    "toBlob":   ADTS_toBlob,    // ADTS.toBlob(source:ADTSFrameArray):Blob
};

// --- implements ------------------------------------------
function ADTS_toBlob(source) { // @arg ADTSFrameArray
                               // @ret BlobURL
    var buffer = [];

    for (var i = 0, iz = source.length; i < iz; ++i) {
        buffer.push(source["stream"]);
    }
...
function _readADTSFrame(source) { // @arg Uint8Array
    ...
    if (VERBOSE) {
        if (layer !== 0x00) {
            console.warn("BAD_FORMAT layer: ", layer);
        }
    }
