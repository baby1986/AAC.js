...
var ADTS = {
    ...
    "toBlob":                   ADTS_toBlob,        // ADTS.toBlob(source:Uint8Array, pad:UINT8 = 0, audioObjectType:AudioObjectType = 2):Blob
    // --- soundless data ---
    "PACKING_A":                PACKING_A,          // AAC-LC 44100Hz
    "PACKING_B":                PACKING_B,          // HE-AAC v1 44100Hz
    "PACKING_C":                PACKING_C,          // HE-AAC v2 44100Hz(22050Hz)
};
...
function ADTS_toBlob(source,          // @arg Uint8Array - ADTS+RawDataBlocks Stream
                     audioObjectType, // @arg AudioObjectType UINT8 = 2 - AAC-LC(2), HE-AAC(5), HE-AAC v2(29)
                     channels,        // @arg ChannelsUint8 = 2 - Monaural(1), Stereo(2)
                     pad) {           // @arg UINT8 = 0 - post-extend packing buffer length.
//{@dev
    $valid($type(source,          "Uint8Array"), ADTS_toBlob, "source");
    $valid($type(audioObjectType, "UINT8|omit"), ADTS_toBlob, "audioObjectType");
    $valid($type(channels,        "UINT8|omit"), ADTS_toBlob, "channels");
    $valid($type(pad,             "UINT8|omit"), ADTS_toBlob, "pad");
//}@dev

    var packing = PACKING_A;

    switch (audioObjectType || 2) {
    case  2: packing = PACKING_A; break;
    case  5: packing = PACKING_B; break;
    case 29: packing = PACKING_C; break;
    }

    var buffer = [source];

    for (var i = 0, iz = pad || 0; i < iz; ++i) {
        buffer.push(packing);
    }
